...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if "Qwen2-VL" in model_id or "Qwen-VL" in model_id:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        def get_per_token_logps(model, input_ids, **kwargs):
            ...
            per_token_p = []
            for logits_row, input_ids_row in zip(logits, input_ids):
                log_probs = logits_row
                token_log_prob = torch.gather(log_probs, dim=1, index=input_ids_row.unsqueeze(1)).squeeze(1)
                per_token_p.append(token_log_prob)
            return torch.stack(per_token_logps), per_token_p
        ...
        per_token_logps, per_token_p = get_per_token_logps(model, prompt_completion_ids, **prompt_inputs)
        # Get rid of the prompt (-1 because of the shift done in get_per_token_logps)
        per_token_logps = per_token_logps[:, prompt_length - 1 :]

        with torch.inference_mode():
            if self.ref_model is not None:
                ref_per_token_logps, ref_per_token_p = get_per_token_logps(self.ref_model, prompt_completion_ids)
            else:
                with self.accelerator.unwrap_model(model).disable_adapter():
                    ref_per_token_logps, ref_per_token_p= get_per_token_logps(model, prompt_completion_ids)
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                ###################add lqb##################################
                # 如果要加熵的比较要在这里加：per_token_logps
                ###################add lqb##################################
                output_reward_func = reward_func(prompts=prompts, completions=completions, per_token_p=per_token_p, ref_per_token_p=ref_per_token_p, **reward_kwargs)
        ...
        # loss += 0.5 * self.layer_in_out_activation_entropy_loss_mask(ref_per_token_p, per_token_p)
    ...
    # 如果要加熵的比较要在这里加：per_token_logps
    ###################add lqb##################################
    def layer_in_out_activation_entropy_loss_mask(self, in_hidden_state, out_hidden_state, label = None):
        """
        :param in_hidden_state(attention的输入)， shape: bs, seq_len, hidden_size
        :param out_hidden_state(attention的输出)
        :return:
        # todo add eps, to avoid overflow；
        信息熵的定义：随机事件为X H(X) = - \sum_{i=0}^n p(x_i)log(p(x_i)); 即每个可能值x_i的概率p与log p的乘积的累加和，再取负数；
        此函数计算输入和输出hidden state之间的熵差；
        和上一个函数的差别就是多了支持label，label中小于0的token对应部分会被忽略
        """
        if label is not None:
            in_hidden_state = in_hidden_state[label>=0]
            out_hidden_state = out_hidden_state[label>=0]
        ...
        entropy_diff = torch.pow((out_entropy.sum(dim=-1) - in_entropy.sum(dim=-1)),2).mean()## attention输出的hidden state与输入hidden state之间的熵差, 会不会有负值？？update：有负值不代表错误，如果加0的截断反而会导致loss一直为0
        return entropy_diff ## 截断操作不能帮助解决nan问题；； 
